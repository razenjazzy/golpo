...
e.preventDefault();
	...
	if($('#system_ip_restriction').is(':checked')){
		var system_ip_restriction = $("#system_ip_restriction").val();
	} else {
		var system_ip_restriction = '';
	}
	var obj = jQuery(this), action = obj.attr('name');
	jQuery('.save').prop('disabled', true);
	$('.icon-spinner3').show();
	jQuery.ajax({
		type: "POST",
		url: e.target.action,
		data: obj.serialize()+"&is_ajax=3&data=system_info&type=system_info&form="+action+'&enable_page_rendered='+enable_page_rendered+'&enable_current_year='+enable_current_year+'&enable_auth_background='+enable_auth_background+'&system_ip_restriction='+system_ip_restriction,
		cache: false,
		success: function (JSON) {
			if (JSON.error != '') {
				toastr.error(JSON.error);
