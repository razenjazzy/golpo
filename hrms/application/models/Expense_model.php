<?php
	...
	public  function approve_expense(){
        $this->db->select('xin_expenses.*,xin_employees.first_name,xin_companies.name');
        $this->db->from('xin_expenses');
        $this->db->join('xin_companies','xin_companies.company_id=xin_expenses.company_id','left');
        $this->db->join('xin_employees','xin_employees.user_id=xin_expenses.employee_id','left');
        $this->db->where('xin_expenses.status=1');
