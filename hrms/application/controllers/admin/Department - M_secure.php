...
class Department extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['tname'] = $this->security->get_csrf_token_name();
		$data['thash'] = $this->security->get_csrf_hash();
	...
	public function add_department() {
		...
		$department_name = $this->Xin_model->clean_post($this->input->post('department_name'));
	...
	public function update() {
		...
		if(is_numeric($keywords[0])) {
			...
			} else if(!preg_match("/^[a-zA-Z ]*$/",$department_name)) {
