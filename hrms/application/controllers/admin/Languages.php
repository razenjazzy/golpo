	...
	public function languages_list()
     {
		...
		$data = array();
          ...
          foreach($language->result() as $r) {
			...
			if($wz_lang == 'english' || $r->language_id=='1' ){
				$del = '';
				$success = $this->lang->line('xin_selected');
			} else {
				
				if($r->is_active==1){
					$success = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_employees_inactive').'"><button type="button" class="btn icon-btn btn-xs btn-outline-success active-lang mr-1" data-language_id="'. $r->language_id . '" data-is_active="0"><span class="fa fa-check-circle"></span></button></span>';
				} else {
					$success = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_employees_active').'"><button type="button" class="btn icon-btn btn-xs btn-outline-info active-lang mr-1" data-language_id="'. $r->language_id . '" data-is_active="1"><span class="far fa-times-circle"></span></button></span>';
				}
			$del = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-outline-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->language_id . '"><span class="far fa-trash-alt"></span></button></span>';
			}  			
			$data[] = array(
				$del,
				$name_flg,
