...
class Expense extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('xin_expenses');
		...
		$data = array();
          ...
          foreach($expense->result() as $r) {
			...
			if($r->billcopy_file!='' && $r->billcopy_file!='no file') {
				$download = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="download?type=expense&filename='.$r->billcopy_file.'"><button type="button" class="btn icon-btn btn-xs btn-outline-success waves-effect waves-light" title="'.$this->lang->line('xin_download').'"><span class="fas fa-cloud-download-alt"></span></button></a></span>';
			}
	...
	public function approve_expense(){
        $data['breadcrumbs'] = $this->lang->line('xin_expenses');
	    $data['approve_expense_list']=$this->Expense_model->approve_expense();
	    //echo '<pre>';print_r($data['approve_expense_list']);
        $data['subview'] = $this->load->view("admin/accounting/All_approve", $data, TRUE);
