...
class Sms_email_manager extends Home
{
    ...
    public function create_email_campaign_action()
    {
        ...
        if(isset($page_auto_id) && !empty($page_auto_id))
        {
            ...
            $sql="SELECT * FROM messenger_bot_subscriber WHERE ".$sql_part." ".$sql_part2." ".$sql_part3." user_id = ".$this->user_id." AND page_table_id = {$page_auto_id} AND is_bot_subscriber='1' AND email !='' AND is_email_unsubscriber='0' AND permission='1' AND unavailable_conversation='0';";
            $lead_list = $this->basic->execute_query($sql);
    ...
    public function analize_custom_api_url_post_method()
    {
        ...
        // $final_string .= '<div class="card" style="padding:20px;box-shadow: 0 4px 8px rgba(0, 0, 0, 0.09);"><label style="word-break: break-all;margin-bottom: 0;"><span style="font-size: 16px;color: #6777ef;padding-right: 10px;margin-bottom: 0;font-weight: bold;"><i class="fas fa-link"></i> '.$this->lang->line("Genarated URL : ").'</span> '.$custom_url."?".$str.'</label></div>';
