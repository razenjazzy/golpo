...
class Messenger_bot extends Home
{
    ...
    public function create_subscriber($sender_id='', $page_id='')
    {
        ...
        $select=array("id","unavailable_conversation","unavailable","subscribe_id","first_name","last_name","status","page_table_id","is_bot_subscriber","user_id");
        ...
        if($latest_question_id!=0 && $invalid_answer==false){
            ...
            if($system_field!=""){
                ...
                if($system_field=="email")
                    $update_data=array("email"=>$answer,"last_update_time" => $now_time);
                    ...
                    $update_data=array("birthdate"=>$answer,"birthdate_entry_time"=>$now_time);

                else if($system_field=="phone")
                    $update_data=array("phone_number"=>$answer,"phone_number_last_update"=>$now_time);
    ...
    public function webhook_callback_main()
    {
        ...
        //$response_raw='{"object":"page","entry":[{"id":"110464057210276","time":1605207280951,"messaging":[{"sender":{"id":"2499471623490220"},"recipient":{"id":"110464057210276"},"timestamp":1605207280756,"message":{"mid":"m_7-Z8gt4n6Lzsrkl634QbuE4zdFjafo41ubxSawTPhZtvCqm4FwYtQ6W1cH964Me19KbhNhVE1wwmaMH6BpzqBA","text":"Red","quick_reply":{"payload":"Red"}}}]}]}';

      // $response_raw='{"object":"page","entry":[{"id":"110464057210276","time":1605360017934,"messaging":[{"sender":{"id":"2499471623490220"},"recipient":{"id":"110464057210276"},"timestamp":1605360017573,"postback":{"title":"Start Flow","payload":"Test Input Flow"}}]}]}';
                ...
                <div class="card card-large-icons card-condensed">
                  ...
                  <div class="card-body">
                    ...
                    <div class="dropdown avatar-badge">
                      ...
                      <div class="dropdown-menu large">';
                        if($this->is_drip_campaigner_exist) { 
                            $middle_column_content .= '<a href="'.$durl.'" class="pointer dropdown-item has-icon card-cta iframed" data-height="795"><i class="fas fa-robot"></i> '.$this->lang->line("Messenger Sequence Settings").'</a>';
                        }
    ...
    public function template_manager_data()
    {
        ...
        {
            $info[$i]["action"] = "<div style='min-width:120px'><a href='#' class='btn btn-circle btn-outline-info get_json_code' title='".$this->lang->line("Get JSON Code")."' table_id='".$value['id']."'><i class='fas fa-code'></i></a>&nbsp;<a class='btn btn-circle btn-outline-warning' title='". $this->lang->line("Edit") ."' href='".base_url('messenger_bot/edit_template/').$value['id']."'><i class='fas fa-edit'></i></a>&nbsp;<a class='btn btn-circle btn-outline-primary' title='". $this->lang->line("Clone") ."' href='".base_url('messenger_bot/clone_template/').$value['id']."'><i class='far fa-copy'></i></a>&nbsp;<a href='#' class='btn btn-circle btn-outline-danger delete_template' title='".$this->lang->line("Delete")."' table_id='".$value['id']."'><i class='fa fa-trash'></i></a></div>";
            $i++;
        }
    ...
    public function get_started_welcome_message()
    {
        ...
        if($ice_breaker_status == '1')
        {
            ...
            $response=$this->fb_rx_login->add_ice_breakers($page_access_token,$ice_breaker_json);
            if(!isset($response['error']))
            {
        ...
        {
            $response=$this->fb_rx_login->delete_ice_breakers($page_access_token);
            if(!isset($response['error']))
            {
                $final_result['ice_breaker_status'] = '1';
   ...
   //=============================DOMAIN WHITELIST================================
    ...
    public function update_subscriber_last_interaction($subscriber_id='',$time='',$conversation_broadcast_unavailable='0',$subsciber_broadcast_unavailable='0'){
        ...
        if($conversation_broadcast_unavailable=='1')
             $update_data['unavailable_conversation']="0";
