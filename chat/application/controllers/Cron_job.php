...
class Cron_job extends Home
{
    ...
    public function conversation_broadcast($api_key="") // braodcast_message
    {
        ...
        if($conversation_broadcast_hold_after_number_of_errors=="" || $conversation_broadcast_hold_after_number_of_errors==0) 
            $conversation_broadcast_hold_after_number_of_errors=10; // default 10
        ...
        foreach($campaign_info_fildered as $info)
        {
            ...
            {
                if($catch_error_count>$conversation_broadcast_hold_after_number_of_errors)  // if 10 catch block error then stop sending, mark as complete
                {
                    $this->basic->update_data("facebook_ex_conversation_campaign",array("id"=>$campaign_id),array("posting_status"=>'4','successfully_sent'=>$successfully_sent,'completed_at'=>date("Y-m-d H:i:s"),"is_spam_caught"=>$is_spam_caught_send,"error_message"=>$error_msg,"is_try_again"=>"0","last_try_error_count"=>$catch_error_count));
                    break;
                }
                ...
                {
                    $this->basic->update_data("facebook_ex_conversation_campaign",array("id"=>$campaign_id),array('successfully_sent'=>$successfully_sent,"error_message"=>$error_msg,"last_try_error_count"=>$catch_error_count));
                }                
            ...
            if((count($campaign_lead)<$number_of_message_to_be_sent_in_try) || $number_of_message_to_be_sent_in_try=="" || $catch_error_count>$conversation_broadcast_hold_after_number_of_errors)
            {
            	$new_posting_status = ($catch_error_count>$conversation_broadcast_hold_after_number_of_errors) ? '4' : '2';
                $complete_update=array("posting_status"=>$new_posting_status,'successfully_sent'=>$successfully_sent,'completed_at'=>date("Y-m-d H:i:s"),"is_spam_caught"=>$is_spam_caught_send,"is_try_again"=>"0","last_try_error_count"=>$catch_error_count);
                if(isset($error_msg))
            ...
            {
                $this->basic->update_data("facebook_ex_conversation_campaign",array("id"=>$campaign_id),array('successfully_sent'=>$successfully_sent,"is_try_again"=>"1"));
            }
        }          
        ...
        foreach ($feed_data as $key => $value) {         
            ...
            foreach($element_list as $key2 => $value2) {
                ...
                if(strtotime($pub_date) > strtotime($last_pub_date)) {
                    ...
                    $media_type = 'PHOTO';
                    $feed_img_url = $this->get_img_from_metatag($post_feed_url);
                    ...
                    if($location_count > 0) {
                        ...
                        $status = $this->_check_usage($module_id=25, $location_count, $user_id);
                        ...
                        if($status == "3") {
                            ...
                            $this->_insert_usage_log($module_id=25,$request=1,$user_id);
                        }
                    }
                }
            }
