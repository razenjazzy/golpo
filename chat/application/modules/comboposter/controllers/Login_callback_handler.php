...
class Login_callback_handler
{
	...
	public function medium($authentication_code)
	{
		// ************************************************//
		$status = $this->comboposter->_check_usage($module_id = 277, $request = 1);
		if ($status == "2") {

		    $this->comboposter->session->set_userdata('limit_cross', 'Sorry, your account import limit has been exceeded for Medium.');
		    redirect('comboposter/social_accounts', 'location');
		    exit();
		} else if ($status == "3") {
		...
		/* get user info */
		$redirect_url = base_url('comboposter/login_callback/medium');

		$error = 0;
		try
		{
		    $access_and_refresh_token = $this->comboposter->medium->get_access_and_refresh_token($authentication_code, $redirect_url);
		    $insert_data['access_token'] = $access_and_refresh_token['access_token'];
		    $insert_data['refresh_token'] = $access_and_refresh_token['refresh_token'];
		}
		catch(Exception $e)
		{
		    $error = $e->getMessage();
		...
		if($error != 0) {
			$this->comboposter->session->set_userdata('account_import_error', $error);
			redirect(base_url('comboposter/social_accounts'),'refresh');
		}
		...
		try
		{
		    $user_info = $this->comboposter->medium->get_user_info($access_and_refresh_token['refresh_token']);
		}
		catch(Exception $e)
		{
		    $error = $e->getMessage();
