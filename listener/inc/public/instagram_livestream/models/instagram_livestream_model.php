<?php
class instagram_livestream_model extends MY_Model {
	...
	public function run( $data ){
		...
		try
		{
            ...
            //Create Live Video
            $response = $this->ig->live->create();
            $broadcast_id = $response->getBroadcastId();
            $stream_url = $response->getUploadUrl();

            $this->ig->live->start($broadcast_id);

            try {
            	if(isset($advance['comment']) && $advance['comment'] != ""){
            		$comment = $this->ig->live->comment($broadcast_id, $advance['comment']);
	            	$comment_id = $comment->getComment()->getPk();
	            	$this->ig->live->pinComment($broadcast_id, $comment_id);
	            }
            } catch (Exception $e) {}
	...
	public function get_posts(  ){
		...
		$this->db->where(" a.time_post <= '".time()."' AND a.category = 'instagram_livestream'");
