<?php
class twitter_profiles_model extends MY_Model {
	...
	public function block_accounts($path = ""){
		...
		$team_id = _t('id');
		$result = $this->main_model->fetch('*', $this->tb_account_manager, "social_network = 'twitter' AND category = 'profile' AND team_id = '".$team_id."'");
		return [
			'position' => 400,
			'id' => $this->module_id,
	...
	public function block_social_settings($path = ""){
		...
		return [
			'twitter' => [
				'position' => 1000,
				'name' => __('Twitter'),
				'desc' => __('Twitter API Configuration'),
