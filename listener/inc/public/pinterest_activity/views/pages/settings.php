...
$comments = pinav($a, "comments");
$directs = pinav($a, "directs");
$reposts = pinav($a, "reposts");
$tags = pinav($a, "tags");
$keywords = pinav($a, "keywords");
$usernames = pinav($a, "usernames");
$blacklist_tags = pinav($a, "blacklist_tags");
$blacklist_usernames = pinav($a, "blacklist_usernames");
$blacklist_keywords = pinav($a, "blacklist_keywords");
$schedule_days = pinav($a, "schedule_days");
$schedule_default="[
...
$slow_level = get_option('pinac_speeds_level', 'slow');

$slow_speed = [
	get_option('pinac_speeds_slow_comment', 1),
	get_option('pinac_speeds_slow_follow', 2),
	get_option('pinac_speeds_slow_unfollow', 2),
	get_option('pinac_speeds_slow_direct', 1),
	get_option('pinac_speeds_slow_repost', 1)
];

$normal_speed = [
	get_option('pinac_speeds_normal_comment', 2),
	get_option('pinac_speeds_normal_follow', 4),
	get_option('pinac_speeds_normal_unfollow', 4),
	get_option('pinac_speeds_normal_direct', 2),
	get_option('pinac_speeds_normal_repost', 2)
];

$fast_speed = [
	get_option('pinac_speeds_fast_comment', 3),
	get_option('pinac_speeds_fast_follow', 6),
	get_option('pinac_speeds_fast_unfollow', 6),
	get_option('pinac_speeds_fast_direct', 3),
	get_option('pinac_speeds_fast_repost', 3)
];
?>
	...
	<form class="ig-ac-body save-action" action="<?php _e( get_module_url("save/".segment(4)) )?>" data-hide-overplay="false" method="POST">
		...
		<div class="ig-ac-options">
			...
			<div class="ig-ac-tab nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
			  	...
			  	<a id="v-pills-keyword-tab" data-toggle="pill" href="#v-pills-keyword" role="tab" aria-controls="v-pills-keyword" aria-selected="false"><i class="fab fa-creative-commons-nd"></i> <?php _e("Keywords")?></a>
