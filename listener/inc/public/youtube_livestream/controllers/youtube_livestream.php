<?php
class youtube_livestream extends MY_Controller {
	...
	public function index($page = "", $ids = "")
	{
		...
		//Sidebar
		$team_id = _t("id");
		$result = $this->model->fetch('*', $this->tb_account_manager, " status = '1' AND team_id = '{$team_id}' AND social_network = 'youtube' ", "social_network, category", "ASC");
		...
		$views = [
			...
			"column_two" => view("pages/general", [ 'file_manager' => $block_file, 'block_post_type' => $block_post_type, 'block_caption' => $block_caption ] ,true), 
	...
	public function save($skip_validate = false){
		...
		validate('null', __('Title'), @$advance['title']);
		...
		if(isset($advance['show'])){
			if(time() + 10 >= $time_post){
				ms([
					"status" => "error",
					"message" => __("Time post must be greater than current time can show schedule to profile")
				]);
			}
		}
