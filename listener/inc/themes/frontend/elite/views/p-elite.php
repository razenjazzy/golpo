<?php 
    ...
    <section class="pricing-area ptb-110 bg-f9f6f6">
        <div class="container">
            <div class="section-title">
                <h2><?php _e("Pricing Plan")?></h2>
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="pricing-table <?php _e( $key%3 == 0 ?"br-left":"" )?> <?php _e( $row->popular==1?"popular":"" )?>">
                        ...
                        <div class="popular"><?php _e("Best value")?></div>
                        <?php }?>
                        ...
                        <div class="price">
                            <span>$<?php _e( $row->price_monthly )?></span>
                        </div>
                        ...
                        <ul>
                            ...
                            <?php foreach ($posts as $value): ?>
                                <li class="<?php _e( isset($row->permissions[ $value['id']."_enable" ]) ? "have":"no" )?>"><?php _e( sprintf( sprintf(__("%s scheduling & report"),  __( $value['group'] ) ) ) )?></li>
                            <?php endforeach ?>
                        </ul>
                        <?php }?>
                        <?php if(!empty($addons)){?>
                        <ul>
                            ...
                            <?php foreach ($addons as $value): ?>
                                <li class="<?php _e( isset($row->permissions[ $value['id']."_enable" ]) ? "have":"no" )?>"><?php _e( $value['sub_name'] )?></li>
                            <?php endforeach ?>
                        </ul>
                        <?php }?>
                        <ul>
                            ...
                            <li class="<?php _e( isset($row->permissions[ "watermark_enable" ]) ? "have":"no" )?>"><?php _e("Watermark support")?></li>
                            <li class="<?php _e( isset($row->permissions[ "file_manager_image_editor" ]) ? "have":"no" )?>"><?php _e("Image Editor support")?></li>
                        ...
                        <div class="footer-p">
                            <a href="<?php _e( get_url("payment/index/".$row->ids."/1" ))?>" data-tmp="<?php _e( get_url("payment/index/".$row->ids."/2" ))?>" class="btn btn-primary"><?php _e("Get Started")?></a>
                        </div>
                    </div>
                </div>
                <?php }?>
